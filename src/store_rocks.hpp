// store_rocks.hpp — declaration of the embedded RocksDB store backend.
// Implementation (RocksInnerImpl) is private to store_rocks.cpp.
#pragma once
#include "store.hpp"
#include <memory>
...
struct RocksInner {
    std::unique_ptr<RocksInnerImpl> p;
    int instance = 0;

    RocksInner(const std::string& path, int inst, size_t cache_entries,
               int64_t cache_ttl_ms);
    ~RocksInner();

    static std::shared_ptr<RocksInner> open(const std::string& path, int inst,
